function Paddle(a,b,d,c,e,f){this.defaultSpeed=f>>7;this.x=a;this.frontX=b;this.y=d;this.width=c;this.height=e;this.minY=this.halfHeight=this.height>>1;this.maxY=f-this.height-this.halfHeight;this.topSection=this.height>>2;this.bottomSection=this.height-(this.height>>2)}function Player(a,b){this.court=a;this.name=b}Player.prototype.givePaddle=function(a){this.paddle=a};ComputerPlayer.prototype=new Player;function ComputerPlayer(a,b){Player.apply(this,a);this.name=b}
ComputerPlayer.prototype.updatePaddle=function(a){return-(this.paddle.y+this.paddle.halfHeight-a.y)>>2};ComputerPlayer.prototype.gameOver=function(a){};
function Ball(a,b){this.court=a;this.ballSize=b;this.halfBallSize=b>>1;this.x=(this.court.width>>1)-this.halfBallSize;this.y=(this.court.height>>1)-this.halfBallSize;this.minX=-this.halfBallSize;this.maxX=this.court.width-this.halfBallSize;this.minY=-this.halfBallSize;this.maxY=this.court.height-this.halfBallSize;this.y_speed=-1;this.x_speed=a.width>>8}
...
this.numPlayers=0;this.game=null;window.message(this.enterMessage);window.court=this;window.draw=1}
...
Court.prototype.draw=function(){for(var a=[],b=0;2>b;b++){var d=this.players[b];if(d){var c=d.paddle;c&&(a[b]=d.updatePaddle(this.ball),0!=a[b]&&(window.draw&&this.context.clearRect(this.paddles[b].x,this.paddles[b].y,this.paddles[b].width,this.paddles[b].height),c.y+=a[b],c.y<=c.minY?c.y=c.minY:c.y>=c.maxY&&(c.y=c.maxY)))}}1<window.debug&&console.log("    elapsed time to clear() and move() paddles: "+(performance.now()-window.start)+" ms");if(this.ball){window.draw&&this.context.clearRect(this.ball.x,
this.ball.y,this.ball.ballSize,this.ball.ballSize);var e=this.ball.getCollisons();this.ball.x+=this.ball.x_speed;this.ball.y+=this.ball.y_speed;window.draw&&this.context.fillRect(this.ball.x,this.ball.y,this.ball.ballSize,this.ball.ballSize);1<window.debug&&console.log("    elapsed time to end of ball.getCollisons: "+(performance.now()-window.start)+" ms");0>e&&(this.context.clearRect(this.ball.oldX,this.ball.oldY,this.ball.ballSize,this.ball.ballSize),-1==e?this.game.point(this.players[1],this.players[0]):
-2==e&&this.game.point(this.players[0],this.players[1]));1<window.debug&&console.log("    elapsed time to end of ball.draw(): "+(performance.now()-window.start)+" ms")}if(window.draw){d=e-1;b=1;do 0==a[b]&&d!=b||this.context.fillRect(this.paddles[b].x,this.paddles[b].y,this.paddles[b].width,this.paddles[b].height);while(0<=--b)}1<window.debug&&console.log("    elapsed time to to end of Court.draw(): "+(performance.now()-window.start)+" ms")};
Court.prototype.startPlay=function(){null==this.game&&(this.game=new Game(this));window.message("");console.log("Starting play");this.paused=!1;this.getCollisons()};Court.prototype.pausePlay=function(){this.paused=!0;console.log("Play paused");window.message(window.court.pausedMessage)};Court.prototype.restartPlay=function(){this.paused=!1;window.message("");console.log("Play restarted");this.getCollisons()};Court.prototype.togglePlay=function(){this.paused?this.restartPlay():this.pausePlay()};
Court.prototype.getCollisons=function(){if(!window.court.paused){if(window.debug){var a=performance.now();console.log("Time since last frame "+(a-window.start).toFixed(2)+" ms");window.start=a}window.court.draw();window.debug&&(a=performance.now(),console.log("court.draw() took "+(a-window.start)+" ms"));window.requestAnimationFrame(window.court.getCollisons)}};window.enableDebug=function(){window.debug=1};window.enableStats=function(){window.stats=!0};
